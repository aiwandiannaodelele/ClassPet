...
file_data = [
    ...
    ("package.json", ["添加了 `@auth/core`, `@auth/prisma-adapter`, `@hugeicons/react`, `@marsidev/react-turnstile` 等依赖。", "更新了 `next-auth` 及 UI 组件相关库版本。"]),
    ("prisma/schema.prisma", ["添加了 NextAuth 所需的 Account, Session, VerificationToken 模型。", "在 User 模型中增加了 role 和 image 字段，并将 email 和 password 改为可选。", "新增了 SystemSetting 模型以存储全局安全设置。"]),
    ("scripts/seedAdmin.ts", ["新增脚本，用于初始化超级管理员账号及全局系统设置。"]),
    ("src/app/(auth)/forgot-password/page.tsx", ["新增忘记密码页面，支持发送密码重置链接。"]),
    ("src/app/(auth)/login/page.tsx", ["增加了 GitHub 和 Google 第三方登录按钮。", "集成了 Cloudflare Turnstile 进行人机验证。", "增加了忘记密码链接及错误提示处理。"]),
    ...
    ("src/components/layout/Header.tsx", ["将头部组件的图标和按钮交互颜色更新为新主题色。", "在用户下拉菜单中为管理员角色增加了进入后台的入口。"]),
    ("src/components/pet/LevelUpAnimation.tsx", ["更新了升级动画的文本和光晕颜色样式。"]),
    ("src/components/pet/PetSelectorDialog.tsx", ["优化了对话框容器样式，移除了不必要的高度限制。"]),
    ("src/components/rule/RuleManagement.tsx", ["移除了分数上限功能及相关 UI。", "简化了生效周期选择，去除了原有的多选按钮组。"]),
...
for comp in ui_components:
    file_data.append((comp, ["使用新的 shadcn-ui 规范更新了组件实现。", "集成了 hugeicons 替换了原有的 lucide-react 图标。"]))

file_data.append(("src/components/widgets/DecibelWidget.tsx", ["调整了分贝指示器的颜色样式以适配新主题。"]))
file_data.append(("src/components/widgets/RandomPickerWidget.tsx", ["更新了随机点名组件的头像框和按钮颜色。"]))
file_data.append(("src/components/widgets/TimerWidget.tsx", ["优化了计时器组件的边框、背景及控制按钮颜色。"]))
file_data.append(("src/lib/auth/authOptions.ts", ["配置 NextAuth 使用 PrismaAdapter，并集成了 GitHub 和 Google OAuth。", "增加了 Turnstile 人机验证校验逻辑。", "在 session 中附加了用户的 role 属性。"]))
file_data.append(("src/proxy.ts", ["移除了未使用的 `isApiRoute` 变量。"]))

# 检查是否还有遗漏的文件
with open('diff_no_lock.txt', 'r') as f:
    lines = f.readlines()

all_changed = set()
for line in lines:
    if line.startswith('diff --git a/'):
        all_changed.add(line.split(' b/')[1].strip())

added_files = set(f[0] for f in file_data)
missing = all_changed - added_files
if missing:
    for m in missing:
        if m == 'components.json':
            file_data.insert(0, (m, ["更新了 UI 组件的主题配置为 `radix-maia`，更改了基础颜色并配置了图标库。"]))

# 按字母顺序或原有顺序排列，保持可读性
file_data.sort(key=lambda x: x[0])

print("此次合并对认证系统进行了重大升级，引入了 GitHub/Google 第三方登录和 Turnstile 人机验证，并添加了忘记密码与重置功能。同时，新增了功能完善的管理后台，并基于新的 `radix-maia` 主题全面重构了整站的 UI 组件和颜色样式。")
print("| 文件 | 变更 |")
print("|------|---------|")
for file, changes in file_data:
    change_str = "<br>".join([f"- {c}" for c in changes])
    print(f"| {file} | {change_str} |")
